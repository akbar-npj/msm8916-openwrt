...
> **UPDATE 2026-09-20 (Doc 147) — this plan rests on a premise that has been retracted.**
> The "approximately 15 minutes (t ≈ 900 s – 914 s)" boundary is **not** a fixed timer.
> Measured stall onsets in a clean boot were **~90 s and ~180 s after boot**, and both
> self-healed within seconds. Measured `a2_power.c:1189` fatal times are 172.353 / 918.195 /
> 1823.753 / 2729.267 s — a *rate*, not a period.
>
> Also retracted here: the `lte_ml1_common_timer.c:390` / `lte_ml1_sleepmgr_stm.c:4054`
> assertions as the crash boundary, and "running `/usr/sbin/qcom-time-daemon` prevents the
> hard reboot".
>
> What *is* established: the reported **crash** is an AP-side NULL `skb` dereference in
> `bam_dmux_send_cmd()` triggered by `wwan0` going down (pstore panic, Doc 147 §4), and the
> residual **stall** is a transient modem-side dormancy/wake gap, not an AP RX-ring defect.
> The AP's RX watchdog never fired during a measured stall (Doc 147 §5.3).
>
> Treat the step-by-step plan below as **historical**. Before acting on any step, re-read
> `Docs/Modem Stability/README.md` and Doc 147.
