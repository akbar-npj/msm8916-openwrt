...
136. **EXPERIMENT DEPLOYED: the coredump capture sits INSIDE the SSR recovery, so it is turned OFF —
     ...
     **Progress: 6 of 20 scored, AP survived.** Fatal #9 (AP `6397.771058 s`, `a2_power.c:1189`)
     recovers in **`0.127579 s`** with **patch 814's rebuild firing and succeeding**
     (`successfully reinitialized BAM channels and rings` → all 8 `CMD_OPEN`s), and fatal #7 (AP
     `6110.407029 s`, sleepmgr) in **`0.119170 s`**, so the six capture-OFF recoveries span
     **`0.119170–0.130237 s`** against **`0.824902–0.831559 s`** for the two with the capture **on** —
     two **non-overlapping** populations **~7× apart**, on six signatures in six recoveries.
     ⚠ **The window now spans TWO regimes** (1 idle-clock fatal + 3 cascade fatals) — say so whenever
