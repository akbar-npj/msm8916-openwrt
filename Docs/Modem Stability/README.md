...
because it takes no lock; it proves
...
119. **TWO CORRECTIONS TO DOC 166 ITSELF, made after its first draft — and they matter more than the
     ...
     **rpmsg/SMD** teardown line on a different thread. Combined, the evidence is that **three
     ...
     `wwan0at0 disconnected`); the region has **at least two adjacent hang points**. **Zero-cost
     discriminator for the next hang: read `/overlay/q6trace.csv` FIRST** — if userspace's CSV
     continues past the last `pmsg` line, the logging path is the stall and the driver locks are
     innocent; if the CSV stops first, it is a global stall. Plus a new instrument (§13 item 3): a
     **non-`printk` liveness beacon**, and the direct test of booting with the serial console removed
     or `console_loglevel = 1`.
