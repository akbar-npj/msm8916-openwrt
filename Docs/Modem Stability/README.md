...
136. **EXPERIMENT DEPLOYED: the coredump capture sits INSIDE the SSR recovery, so it is turned OFF —
     ...
     **Progress: 4 of 20 scored, AP survived.** The fourth sample (fatal #6, AP `5208.721361 s`,
     `lte_ml1_sleepmgr_stm.c:4054`) recovers in **`0.127373 s`**, so the four capture-OFF recoveries span
     **`0.124531–0.130237 s` (spread 5.7 ms)** against **`0.824902–0.831559 s` (spread 6.7 ms)** for the
     two with the capture **on** — two **non-overlapping** populations **6.6× apart**. The third sample
...
138. **★ THE FIRST AP-SIDE OBSERVABLE THAT PRECEDES A MODEM FATAL: every `a2_power.c:2949` is preceded
     ...
     **0.132 suspends/s** and `pc_state=0` in 6 of 10 samples; during **364.34 s of 1 Hz traffic
     ...
     S2** (`/overlay/s2.sh`, written and syntax-checked, **not yet run**) — one ping every 15 s × 80,
     recording each ping's RTT and the counters either side; **P6:** pings whose wake missed the edge
     show an RTT spike up to the 100 ms watchdog period; **P6-FALSIFIER:** no RTT difference. S2 also
     regression-tests patch 812 at exactly the idle period Doc 156 measured as broken.
     Evidence:
