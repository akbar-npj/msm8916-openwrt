...
56. **`ioremap` succeeding is NOT permission to read.** Doc 153 named "a `nomap`-capable kernel module"
    ...
    running** — no panic.
57. **Why: mpss is protected by TrustZone, not by the kernel.** `q6v5_xfer_mem_ownership()`
...
58. **This re-reads Doc 153's `/dev/mem` result.** `read()` → `EFAULT` and `mmap()` → `SIGBUS` are the
...
59. **Why `rpmring` works and this does not.** The RPM log ring is in **SRAM** — a normal reserved
...
60. **The "watch the fatal develop live" idea is dead.** The coredump is taken *after* the driver hands
...
61. **A second hazard: the module could not be unloaded and debugfs wedged.** After the aborts,
    ...
    **`D` state** (`ls /sys/kernel/debug/` hung too). The rest of the system was unaffected — the soak
    ...
    abort while holding a `debugfs_file_get()`. **A reboot clears it completely** (verified).
