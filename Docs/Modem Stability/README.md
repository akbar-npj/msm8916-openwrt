...
138. **★ THE FIRST AP-SIDE OBSERVABLE THAT PRECEDES A MODEM FATAL: every `a2_power.c:2949` is preceded
     ...
     **⚠ AND IT IS NOT SUFFICIENT — 3 of 8.** Eight resyncs were observed in total; five had **no**
     `a2_power.c:2949` after them (boot R's was followed by a *different* fatal 73.47 s later; boot S's
     four had none before its log ended 119 s later). So **`a2_power.c:2949` ⇒ resync is 3/3** while
     **resync ⇒ `a2_power.c:2949` is 3/8**, and the second number is the one that decides whether a fix
     is possible. **A lag this tight feels like a cause; the denominator is what keeps it honest.**
