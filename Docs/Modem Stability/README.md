...
136. **EXPERIMENT DEPLOYED: the coredump capture sits INSIDE the SSR recovery, so it is turned OFF —
     ...
     **Progress: 2 of 20 scored, AP survived — and the second sample (fatal #4, AP `3477.969305 s`,
     `a2_power.c:2949`) reproduces fatal #3 to within 5 ms**, so the A/B no longer rests on one window:
     ...
     **4 fatals / 5 `MBA booted` (cold boot + one per recovery) / 2 `port failed halt`**, and the
     ledger's **independent** `coredumps` column (a separate 10 s poller, not the driver) reads
     **17, 18, 18, 18** — fatal #1 took it 16→17, fatal #2 17→18, **#3 and #4 produced no dump at
     all**. Fatal #4's interval was **759.53 s**, off the 902.4 s clock — which is *expected*, not an
     anomaly: Doc 162's taxonomy puts `a2_power` at 68.5–941.3 s and explicitly **not** on the clock.
     **Stated
...
138. **★ THE FIRST AP-SIDE OBSERVABLE THAT PRECEDES A MODEM FATAL: every `a2_power.c:2949` is preceded
     ...
     **⚠ AND IT IS NOT SUFFICIENT — 3 of 13, AND THE PRE-REGISTERED P2 IS WHAT MEASURED THAT.** Pooling
     every boot with a surviving log (**P 1/1, Q 1/1, R 1/0, S 4/0, Y 6/1**): **`a2_power.c:2949` ⇒
     resync is 3/3** while **resync ⇒ `a2_power.c:2949` is 3/13** — the two figures now differ by more
     than **4×**, and the second is the one that decides whether a fix is possible. *Caveat that cuts
     the other way:* the P/Q/R resync counts come from the partial `q6trace.log`, so a missing resync
     would make the denominator **larger** — 3/13 is a floor. **A lag this tight feels like a cause; the
     denominator is what keeps it honest.** **AND THE RATE IS NOT STATIONARY:** this boot had **1**
     resync in its first 3478 s, then **5 in 145 s** starting at `4079.094133` — with no AP-side change
     (no patch, no reboot, no manual action, `coredump` still `disabled`, `rproc=running`), `pc_quiesce_ms`
     down to **1669 ms** (the modem cycling, not quiesced). **Recorded with a pre-registration:** **P3** —
     the storm is the modem approaching its next fatal, so fatal #5 should be a
     `lte_ml1_sleepmgr_stm.c:4054` at roughly AP **4380 s** (3477.969 + 902.4); **P3-FALSIFIER** — no
     fatal in ~15 min, or a fatal with no storm in front of it, or a storm that just stops. **If the
     storm precedes the fatal, the right window is MINUTES, not milliseconds**, and the three tight hits
     are the tail of a storm that happened to be caught — a different mechanism from "the resync causes
     the fatal". **§8.15's "cheapest next step" is done and the answer is NEGATIVE:** `pc_timeout_count`
     counts **the AP waiting for the modem** (a 250 ms `wait_for_completion_timeout(&dmux->pc_ack_completion)`
     and a 1000 ms `wait_event_timeout(dmux->pc_wait)` inside `bam_dmux_runtime_resume()`, patch 808) —
     the *mirror* of Reading C, so **it cannot discriminate C; do not design around it**. And the
     post-fatal `pc-ack timeout` lines are **recovery artefacts, not precursors**: #1 → +0.431112 s,
     ...
     **What it does NOT claim:** the resync does not (yet) cause the fatal, and it explains **neither**
     ...
     seen with a resync in front of it**. Evidence:
