...
136. **EXPERIMENT DEPLOYED: the coredump capture sits INSIDE the SSR recovery, so it is turned OFF —
     ...
     **Progress: 3 of 20 scored, AP survived — and the third sample (fatal #5, AP `4304.991306 s`,
     `a2_power.c:1189`, a THIRD signature) reproduces it again at `0.124531 s`.** The second sample
     ...
     **5 fatals / 6 `MBA booted` (cold boot + one per recovery) / 2 `port failed halt`** (exactly the
     two capture-ON recoveries), trace `01-09` ×7 / `10-23` ×8, `coredump_live` **frozen at 18** — and
     the ledger's **independent** `coredumps` column (a separate 10 s poller, not the driver) reads
     **17, 18, 18, 18, 18** — fatal #1 took it 16→17, fatal #2 17→18, **#3, #4 and #5 produced no dump
     at all**. Fatal #4's interval was **759.53 s** and fatal #5's **827.02 s**, both off the 902.4 s
     clock — which is *expected*, not an anomaly: Doc 162's taxonomy puts `a2_power` at 68.5–941.3 s
     and explicitly **not** on the clock. **Stated
...
138. **★ THE FIRST AP-SIDE OBSERVABLE THAT PRECEDES A MODEM FATAL: every `a2_power.c:2949` is preceded
     ...
     **★ AND THE STORM IS AP-RUNTIME-PM-GATED, WHICH MAY BE THE MOST ACTIONABLE RESULT OF THE ROUND.**
     A lost edge needs `pc_state == 0` **and** the line asserted, so it can only happen on the way **out
     of** a quiesced state. **S1 (600 s of 1 Hz pings, pre-registered as P5) tests that directly —
     interim, ~110 s in: `pc_resync_count` frozen at 20 and `pm_suspend_attempts` frozen at 636, i.e.
     ZERO new resyncs and ZERO new suspends against a pre-S1 rate of 1 resync per 36.3 s.** If it holds,
     the storm, the **data stall** (the first packet after idle) and the PC-handshake failures are
     **one defect family — the idle→active transition** — and an idle-avoidance policy becomes a
     candidate **mitigation needing no firmware change**. **Caveat:** Doc 162 measured a **substituted**
     `a2_power.c:1189` at ~895 s *under* 1 Hz traffic, so traffic is **not** expected to suppress the
     fatals — which cleanly separates the storm (AP-side, PM-gated) from the fatals (modem-internal).
     Evidence:
