...
137. **⚠ THE HOST KERNEL LOG REFRAMES THE AP-RESET RATE: §8.10's "reset that survived 823" WAS MY OWN
     HUB INSTALLATION, AND AT LEAST 6 OF 19 OUTAGES ARE NOT AP HANGS (Doc 170 §8.14).** §8.10 blamed
     ...
     **The 16:40–16:46 "boot loop" is a HOST controller episode.** The watcher saw uptime
     `2427 → 46 → 38 → 79` (three AP reboots in seven minutes); the host log shows **xHCI ×4 / ×12 /
     ×24 / ×16 and root-hub disconnects immediately before each**, inside the hour holding **36 of the
     day's 48 `xHCI Host Controller` re-registrations** (plus 12 `usb usb1: USB disconnect` — the root
     hub — over the day). **The host reset the bus; the dongle rebooted as a consequence.**
     ...
     **Classified: ≥6 of the 19 outages are not AP hangs** — 4 host-controller-caused (16:40:47,
     ...
     have no host-side trigger and stay candidates, but **"no host trigger" is not proof** — the host
     log only records what the *host* saw. Conversely the host saw **39** dongle re-enumerations against
     ~19 AP uptime-decreases (a clean re-enumeration at 14:39 with **no** AP reboot), so
