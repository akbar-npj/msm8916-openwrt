...
136. **EXPERIMENT DEPLOYED: the coredump capture sits INSIDE the SSR recovery, so it is turned OFF —
     ...
     **3–5 %** — against which `P(0 reboots in 21) = 0.34–0.53`, so **21 clean SSRs does NOT establish
     ...
     Elec Cause 0x0` (an SMC ELECTRICAL EVENT) plus `xhci-hcd … remove, state 4` / `USB bus 1
     deregistered`, i.e. the host re-probed its **controller**. **An AP reboot cannot do that** — it
     only disconnects the USB *gadget* on the bus already present. So v1's `*** FAILURE` was wrong.
     **PRE-REGISTERED ADJUDICATION RULE (v2, both tests required for a failure): (A) the host log over
     the outage window shows no `USB bus N deregistered` / `Elec Cause` / `xhci-hcd … remove`, AND
     (B) the dead boot's `dmesg_roll_<boot_id>.txt` ends with a fatal and NO completed recovery after
     it** — because an AP hang happens *inside* a fatal-triggered SSR, so a real reset must leave the
     SSR unrecovered (verified on boot B: last fatal line 191, last recovery line 239 ⇒ not scored).
     **An unavailable `dmesg_roll` is UNKNOWN, not a negative.** Watcher: `scratch/watch813_v2.sh 17`.
     **Boot A (`a9fd907c`, 17 fatals) and boot B (`ea6401be`, 2 fatals at `920.32`/`1822.54`, both
     `sleepmgr:4054`, `coredumps` frozen at 18) are pooled with the bar 15 + 2 + 3 = 20, pre-registered
     before the run** (PART 20); boot B/C are the **cleaner** regime — `/overlay/coredump_ENABLE` is
     absent, so `rc.local` disables the capture at boot and **every** fatal counts. **Fatal #14 (AP
     `10210.922046`) is
...
| `171_RPM_MASTER_STATS_ENABLER_FOR_OPENWRT.md` | **PREPARED, NOT BUILT, NOT FLASHED — the enabler that lets OpenWrt measure the modem's own RPM power-collapse rate (Task 100).** Three edits: NEW patch **824** (DT), `CONFIG_QCOM_RPM_MASTER_STATS=m`, and a `KernelPackage/rpm-master-stats` with **no `AUTOLOAD`** (the driver ships no `MODULE_DEVICE_TABLE` on purpose — "to be loaded manually only"), so the counters appear at `/sys/kernel/debug/qcom_rpm_master_stats/{APSS,MPSS,PRONTO}` after `modprobe rpm_master_stats`. **The offsets are read out of the STOCK HMU05 DTS, not inferred:** `qcom,rpm-master-stats@60150` with `qcom,master-offset = <0x1000>` over `"APSS","MPSS","PRONTO"` ⇒ **0x60150 / 0x61150 / 0x62150**, i.e. offsets 0x150/0x1150/0x2150 inside `rpm_msg_ram` — **independently corroborated by upstream msm8974, whose APSS slice is also `sram@150`**, and by the driver's `sizeof(struct rpm_master_stats)` = **80 B (0x50)**, the size used here instead of upstream's incorrect 0x14. Reading the driver's `probe()` (not its name) is what forces **three separate nodes**: it resolves each `qcom,rpm-msg-ram` phandle at *address index 0*, and it wants `qcom,master-names` — **not** the downstream `qcom,masters` in the stock DTS, which would make the probe fail. Patch verified by `patch -p1 --dry-run` against the live post-823 tree; **no build, no flash, no on-device verification**, and the counters are NOT claimed to be non-zero. **Known confound:** Android's 1.310–1.711/s MPSS rate was measured on a MOVING BUS, so it is a bus baseline — this enabler only becomes comparable after a STATIONARY Android re-measurement. |

| `172_THE_TIME_DAEMON_DIFFERENTIAL_AND_THE_MODEMS_OWN_CLOCK.md` | **Both stacks run a time daemon — the differential is architecture, persistence and policy, not presence.** Android = `time_daemon` (PID 243) **+ `com.qualcomm.timeservice`**, talking over a local `#time_genoff` socket, touching ATS bases **1, 2 and 13**, **persisting `/data/time/ats_N` = (AP wall − AP RTC)**, reading `qpnp_rtc` at boot and calling `settimeofday`. OpenWrt = one process, QRTR only, **base 2 only**, **persists nothing** (`/var/run` is tmpfs), never touches the RTC or the system clock. **★ The instrument nobody had used:** the daemon's once-a-minute line reads *both* operands from the modem, so `ATS_RTC` (base 0, QMI TIME service 22) **is the modem's own uptime counter** — and it puts the fatal at **900.427 s** of modem uptime, from two routes that agree to **28 ms**, and **59 ms** against the AP-derived borrowed-offset anchor, i.e. **two instruments sharing no clock agree to <0.1 s** (honest interval `[899.4, 901.4) s` from the 1 s stamp quantization). That is a 4th sample of Doc 164's short `sleepmgr` band (900.5/900.662/900.965) obtained **without borrowing an offset**. The **epoch gap is exactly 902.470 s**, computed with **no AP clock at all**. **And the retracted premise is falsified from its own side:** the captured boot ran `-r 60`, the daemon wrote `ATS_USER` to the modem **14 times**, the last one verified **55 s before the fatal** — and the fatal fired anyway. Zero daemon timeouts *before* the fatal (no precursor); after it the daemon is a clean SSR detector (port gone at +0 s, service discoverable at +1 s, **not responsive until +4 s**, then self-recovers). |

| `173_ATS_RTC_READONLY_POLL_IMPLEMENTED_AND_VERIFIED.md` | **Implemented and verified; DISABLED by default on purpose; NEVER RUN against a real modem.** Doc 172 §7 item 1: give OpenWrt a periodic read-only poll so `ATS_RTC` (the modem's own uptime) stops going dark under `-r 0`. **The `-p` flag was DEAD** — `case 'p': enable_periodic_get = false;` assigned the flag its own default, so `validate_periodic_time_get()` was unreachable in every build and the loop comment claiming it "can be explicitly enabled with '-p'" was false. Fixed, and pointed at base 0 via a new `log_modem_uptime()` that has **no state effects** (unlike the liveness check, which can revoke sync). **The read-only property is a property of the MESSAGE, not of our care:** `qmi_encode_message()` returns **14** for the GET and **25** for the SET — a **11-byte** delta that *is* the u64 offset TLV — verified by **running the real libqrtr encoder** (static aarch64, libqrtr built from source, executed **natively**) and matching it against the deployed wire, where **all 38 GETs are exactly 14 bytes and all 19 SETs exactly 25, one distinct length each**. The new poll calls the **same helper + base** that already produced **15 successful `GET(base 0)` reads** in the frozen capture (every printed `RTC (…)` non-zero, `RTC (0)` zero times). **Three negative controls, all firing:** injected SET call, a GET rewritten to 25 bytes, and a GET descriptor given an offset TLV. `gcc -Wall -Wextra` clean. **NOT verified: it has never executed against a modem** (device is on Android 4.4.4), the 60 s cadence's power cost is unmeasured, and the init script's commented line is untested as a procd invocation. Kept off because Doc 146 §9's fatal-rate experiment is still running. |
| `174_STATIONARY_ANDROID_RPM_BASELINE.md` | **Task #101's Android half is CLOSED, and the bus caution is measured FALSE.** Two independent stationary boots give the modem's own power-collapse rate as **1.3831 /s** (324 s, 448 collapses) and **1.5137 /s** (456 s, 690 collapses), wall period **723 / 661 ms**, with **APSS and PRONTO flat** — only MPSS moves. 50 s windows fluctuate **1.09–1.92 /s** on both, so the 9.4 % gap is spread, not a boot effect. **★ The correction:** Doc 171 called Android's 1.310/1.523/1.711 /s a BUS baseline, but the stationary values **fall inside the bus range** — the bus did not confound this metric. **★ Two new capabilities:** Android is **32-bit `armv7l`, kernel 3.10.28, no `/system/lib64`**, so every static aarch64 binary dies with `not executable: magic 7F45` (not a permission problem — `/data` has no `noexec` and the md5 matches); and **the RPM log ring IS readable on Android via `/system/xbin/devmem`** at the same `0x29dc58` (+ counter `0x29dc38`) with the same grammar (3584/3584 markers at stride 8), giving an Android census of `ldoa` 37.9 %, `bslv` 19.7 %, `bmas` 14.4 %, `clk2` 6.9 %, `smpa` 6.0 %, `clk1` 5.8 %, `clk0` 5.2 %, `clka` 4.1 %. **⚠ Two honest negatives:** the RPM record rate (Android ~540–590 /s) **overlaps** OpenWrt's 224 baseline and Doc 170's 470–830 windows, so it is **not** a clean differential; and the resource-mix outliers (`clk0` 14.7×, `smpa` 0.38×) are compared against a **fatal-adjacent** OpenWrt window, so **no story is offered**. **★ A trap this doc caught in itself:** `shutdown_req`/`wakeup_ind` are 19.2 MHz counters that **wrap 2³² in 223.7 s**, so a 324 s delta reads as a bogus "32.4 % duty cycle"; the 10 s deltas give observed/expected **p50 = 1.00 on both counters on both boots** ⇒ **free-running, not gated**, and the duty-cycle/period figures were retracted before publication. Also attributes the mid-run disconnect to a **loose cable** from the host log (`xhci … remove` + `USB bus N deregistered` + `Elec Cause 0x0`) — the third host-side power event, and **not** an AP hang. **★★ §9 WITHDRAWS THE CORPUS'S MCPM "2.3×" CLAIM — and §9.8/§9.9 then corrected §9's OWN first two reasons for it.** `Stock_Android_Live` §2's *Android 1.88 /s vs OpenWrt 0.83 /s* is **not established**, because **the rate is set by the TRAFFIC CONDITION and both sides of that comparison are single windows whose traffic was never controlled or recorded.** Seven stationary Android windows on one boot: **base traffic, verified** (C/D/E/F, 80–99 min) = **2.071–2.125 /s, spread 2.6 %**; **no traffic, verified** (G, 116–120 min) = **1.683 /s**; unrecorded (A 37–42 min) 1.629; unrecorded (B 62–64 min) 2.165. So the rate is **reproducible to ~3 % at a fixed traffic condition, ~20 % lower with no traffic, and NOT a function of session time** — which also explains A's low value (low traffic, not an early-boot transient). **Two earlier reasons are retracted in place:** §9.1's ("two draws differ 1.47×") was **mostly a denominator artifact** (pre-history), and §9.8's first draft ("drifts ~30 % across a session") was **falsified by capture G**, a late window that came in LOW. The mechanism is that the cadence is **quantized to 0.320 s (3.125 Hz) and its multiples** — 0.64 s (2×), 1.28 s (4×) — so the mean is `3.125/s × (fraction of time in the fast mode)`, a **duty cycle**; the modal interval is **0.320 s in all seven captures** and **68.5–77.1 %** of intervals land on a multiple within ±5 ms. **What is comparable across platforms is the modal interval, not the mean.** **★ Two instruments:** every `FW_WAKE-UP_Start` embeds its own `Start time`/`End time`/`duration`, and intersecting `d = floor((End-Start)/R)` over all frames pins **R ∈ (19.1818, 19.2000] ticks/µs** (506/506 and 295/295 frames), excluding 21.1 MHz — the same 19.2 MHz domain as the RPM log, and **wall-accurate** (capture B: 136.3 s modem span vs a 134 s file). **⚠ And a denominator trap it caught:** a qmdl can carry **PRE-HISTORY** — DIAG records the modem buffered while no `diag_mdlog` was attached and flushed at attach (+29 / +32.3 / +36.3 s measured) — and because that interval is **IDLE it LOWERS the mean** (0.88 /s first bin vs 2.0–2.2 after); it alone moved the four-capture spread from 2.6 % to 14.1 %. **⚠⚠ §9.9's tooling findings, which are the most reusable part of this doc:** the **traffic control never actually held in ANY capture** — reading the ping logs afterwards showed C, D, E and F all ran the *identical* base cadence (F was never `heavy`, E never `none`), so **never trust the launch, read the traffic log**; and a **background job from a short-lived `adb shell` dies, with `setsid` rescuing `diag_mdlog` but NOT a `sh -c` loop** — the launcher that works is `adb shell 'loop' &` backgrounded on the **host**; plus `ps` renders the wrapper as plain `sh` (name greps are blind), a one-shot ping count false-alarms, and Android **recycles pids** so a stale pidfile can `kill -9` an unrelated process. |
| `175_OPENWRT_RPM_BASELINE.md` | **Task #101's OpenWrt half is CLOSED.** The Doc 171 enabler (kmod-rpm-master-stats + DT node patch 824) is built, flashed, and verified on device: `modprobe rpm_master_stats` creates `/sys/kernel/debug/qcom_rpm_master_stats/{APSS,MPSS,PRONTO}`. **Capture 3 (clean, zero fatals, 602 s):** MPSS rate **1.5547 /s** — only **2.7 % above** the Android range (1.3831–1.5137 /s), 3.5× smaller than the Android boot-to-boot gap (9.4 %), inside the Android sub-window spread (1.09–1.92 /s). 50 s sub-windows: 1.022–1.744 /s, spread 46.2 %. **Capture 1 (confounded by 2 fatals at 109/388 s):** whole 1.68 /s (ABOVE 10.9 %), pre-SSR 1.25 /s (BELOW 9.6 %, inside sub-window spread), post-SSR 1.73 /s (inflated by reconnection). **Conclusion:** the RPM MPSS collapse rate is **NOT a platform discriminator** — same finding as Doc 174 §9.9 for MCPM: the rate is set by the **traffic condition**, not by the platform. Also: capture 2 was lost to a **loose-wire reboot** (the fourth host-side power event); `/dev/diag` does NOT exist on OpenWrt so MCPM cadence (Task #104) cannot be measured the same way as Android without building a DIAG kernel module. |
| `176_FAST_DORMANCY_DOWNGRADED_WDS_GO_DORMANT_UNSUPPORTED.md` | **The Doc 170 PART 34 FastDormancyService lead is DOWNGRADED, not confirmed.** The whole chain was traced statically — `FastDormancyService.enterDormancy()` → `QcRilHook.qcRilGoDormant()` → `QCRILHOOK_GO_DORMANT 0x80003` → (`strings` on `libril-qc-qmi-1.so`) `qmi_wds_go_dormant_req` → **QMI WDS 0x0025** — and the message is refused on device: `qmicli -p -d /dev/wwan0qmi0 --wds-go-dormant` → **error 25 `DeviceUnsupported`** (positive controls `--wds-get-dormancy-status` / `--wds-go-active` succeed, so the failure is specific to 0x0025). Two misreads corrected: `sent:N received:M` is `TrafficStats.getMobileTx/RxPackets()` off the **local** `/proc/net/dev`, not modem messages; and `isAlarmExpired: false` in **all 284** samples means the 15 s alarm never expired, so the service **never triggered**. The falsifier run stands (OpenWrt's only periodic AP→modem send is WDS 0x0024 every 30 s) but the A/B it motivated is **not** warranted. |
| `177_THE_PERIOD_IS_AN_ALWAYS_ON_CLOCK_NOT_MODEM_UPTIME.md` | **Doc 149 §2 IS INVERTED: the ~903.675 s fatal interval is anchored to a clock that SURVIVES the modem SSR, and the “modem-uptime period” is an artifact of the recovery time.** Re-analysing Doc 149’s **own** table: `corr(R,T) = −1.00000`, `sd(AP) = 0.40 ppm` vs `sd(T) = 57.6 ppm` (the T-invariant prediction is wrong by **143×**). Live 2026-09-22 boot (9 fatals): `corr(R,T) = −0.882`, `sd(AP) = 3.1 ppm` vs `sd(T) = 4.9 ppm`. Cross-boot: **AP agrees to 0.65 ppm** (903.674618 vs 903.675206 s) while T moves **563 ppm**; R changed **+0.507 s** and T changed **−0.508 s**. So the timer’s beat is unchanged while the modem is **down** for ~0.9–1.4 s ⇒ an always-on time base (AON/XO/RPM), not the modem’s resettable clock. Kills the `400 × 2.256 s = 902.4 s` reading (live T = 902.775 s, **415 ppm** off). Pre-registers **P1** (next boot: AP 903.675 ± 0.005 s) and **P2** (a forced non-fatal SSR discriminates free-running vs armed-at-boot). |
| `178_A2POWER_IS_THE_BEAT_DELAYED_BY_AN_RPM_STALL.md` | **Doc 177 §7's pre-registered P1 is NOT MET — and the failure has a structure.** Fatal #10 landed at AP **8719.020382 s** (`a2_power.c:1189`), **+58.682 s** past the predicted beat (8660.337898), interval 962.358 s. The `rpmring` log then shows the RPM **stalled 60.314 s**, from **8658.929** to **8719.243** — i.e. **stall_start = beat − 1.409 s** and **fatal = stall_end − 0.223 s**, with the ring advancing only 7 072 words where ~470 000 were due (**96 % stalled**). **H1: the ~903.675 s timer expires ON SCHEDULE; the `a2_power` assert surfaces only when the RPM stall clears, so dmesg time = beat + stall** — which is what Doc 149/165's `rpm.sync` stall already predicts from the other direction. **NOT established: n = 1**, and the control (`rpm11`, 2026-09-21, 1451 s, same analyser) shows **>2 s RPM stalls are routine — 161 of them, 38.9 % of the span, max 13.26 s** — so only the live capture's **concentration** (78 % of its stall time in the one 60 s fatal window; max gap 2.0× the control's) is anomalous. The instrument (50 ms `/dev/mem` polling; the modem woke 133 s in) is not fully cleared. Pre-registers **P2′** (excess ≈ stall for every future `a2_power` fatal) and **P3′** (a 50 ms-polled idle window with no fatal). |
| `179_THE_FATAL_OFFSET_IS_A_CARRIED_STATE.md` | **Doc 178's P2′/P3′ are scored — P3′ CONFIRMED, P2′ CORRECTED, and Doc 178's H1 needs its framing fixed.** `P3_PREREG.md` was written **while the capture was still running**; the 861 s capture then delivered **fatal #11 at 9622.891300 s** (`lte_ml1_common_timer.c:390`). **(a)/(b) FALSIFIED** — no fatal at the free-running beat 9560.176 (device reached 9609 with count still 10); **(c) CONFIRMED to +0.195 s** (predicted 9622.696). **The offset is a LATCHED STATE, not a per-fatal delay:** it sits at ≈0 for #2–#6, jumps to **+3.74** at #7, holds at **+3.84** through #8/#9, jumps to **+62.52** at #10, and holds at **+62.72** at #11 — so **the signature does NOT determine lateness** (#11 is `common_timer` and 62.7 s late). The shift ≈ collapse **+ ~2.2 s** at both #10 and #11 (unexplained). **P-STALL CONFIRMED (n=1 vs 1):** the `a2_power` fatal #10 had the 60.3 s collapse; the `common_timer` fatal #11 had **none** (max gap 1.6 s, bracketing it). **P3′ CONFIRMED — the instrument is CLEARED:** an 861 s 50 ms-polled window gives max gap **2.527 s**, 3 gaps >2 s (0.9 %), **0** bins <20 rec/s, vs the fatal-#10 capture's 26.0 s / 19.0 % / 22 rec/s. **What the collapse IS:** splitting transactions by size shows the ≥50-record **full-resource-set batches fall to exactly 0.00/s** for the whole 60.3 s while the modem's 9-record client-1 requests continue at 0.35/s and the RPM's 19.2 MHz clock runs normally — i.e. **the collapse is the power-collapse re-evaluation stopping, which is the `a2_power` root cause ("sleep count not incrementing") observed directly**, starting 1.4 s before the beat. **Four instrument corrections:** (1) `word[2]` is **NOT a type** — it is the **high word of a 64-bit 19.2 MHz counter** (verified 4/4 at the `word[1]` wrap); (2) the RPM log clock runs **19,211,535 Hz = +601 ppm** vs `CLOCK_MONOTONIC`; (3) the 256-record ring is **undersized vs the RPM's ~300/~560-record bursts** (peak ~9,200 rec/s) so the log loses **13.4 %** (46 overruns, 152,715 of 176,432) — absolute counts are lower bounds; (4) **Doc 178's cross-clock comparison is SAFE** — `/dev/kmsg` markers show `CLOCK_MONOTONIC` vs printk offset constant to 0.2 ms over 30 s (**<7 ppm**). Pre-registers **P4** (next `a2_power`: ≥50-record txns → 0.00/s, ±2 s), **P5** (offset latch), **P6**, **P7** (`-i 20` should remove the 13.4 % loss). |
| `180_THE_PERSISTENT_ATS_STATE_PATH.md` | **Implemented and verified by a native aarch64 test (54 checks, 0 failed); NEVER RUN against a real modem.** Closes the persistence half of Doc 172's differential: Android keeps `/data/time/ats_<N>`, the port kept **nothing** (its only file was the tmpfs `/var/run/qcom-time-synced`). New overlay-backed record at `/etc/qcom-time/ats` (`-P <path>`, `-P none` to disable), written **atomically** (tmp + fsync + rename + dir fsync), holding `base/offset_ms/rtc_ms/wall_ms/delta_ms/synced_at`. Two uses: **(1) reset vs reboot with NO AP clock in the arithmetic** — `ATS_RTC` only moves forwards while the modem runs, so `rtc_now < rtc_recorded` proves the modem restarted and `>` measures how much modem time elapsed across the AP's downtime (the project previously needed the HOST's `journalctl` for this); **(2) restoring the offset when the AP wall clock is unusable**, behind two guards (≥2020, and not behind the recorded wall) and **never overriding a plausible clock**, because a legitimate NTP step and a broken boot clock are indistinguishable in one sample. Also fixes a latent defect: the NITZ-not-locked fallback computed `ap_time_ms - GPS_EPOCH_OFFSET_MS` in `uint64` with no floor, so an unset clock **underflowed to ≈1.8446e19 ms** and passed the `>= rtc_val` test — the daemon would have sent that to the modem. It now refuses instead. **Wire-neutral: adds NO QMI message** (filled from the ATS_RTC/ATS_TOD reads the handshake already makes) and sizes are unchanged (GET 14 B, SET 25 B), so Doc 173's read-only property and Doc 146 §9's idle-traffic experiment are untouched. Writes flash once per boot plus once per SSR; an unchanged REFRESH never rewrites. NOT established: no live run, the verdict has **no live control**, and the ~903.675 s clock / latched offset of Doc 179 are untouched. |
