...
function handleOpen(ev) {
	if (ev === 'ropenissues') {
		window.open('https://github.com/4IceG/luci-app-sms-manager/issues');
		return;
	}
	if (ev === 'opendiscussion') {
		window.open('https://github.com/4IceG/luci-app-sms-manager/discussions');
		return;
	}
	if (ev === 'ropencoffee') {
	...
	if (ev === 'opentopic') {
		window.open('https://eko.one.pl/forum/viewtopic.php?id=20096&p=10');
		return;
	}
}
...
return view.extend({
	...
	render: function(data) {
		...
		m = new form.Map('sms_manager', _('Package update and support'), info);
		s = m.section(form.TypedSection);
		s.anonymous = true;
		s.tab('info', _('Luci-app-sms-manager Info'));
		
		let packages = [{
			package: _('Luci-app-sms-manager'),
		...
		for (let i = 0; i < packages.length; i++) {
			...
			for (let j = 0; j < pkg.buttons.length; j++) {
				...
				if (btn.disabled) {
					...
					buttonAttrs.click = function() {
						return handleOpen(btn.action);
					};
				}
